...
SQLiteDB::SQLiteDB(std::string _filename){
	...
	/*
	rc = sqlite3_exec(db, "PRAGMA journal_mode=MEMORY;", 0, 0, 0);
	if (rc != SQLITE_OK ) {
	...
	*/
	...
	char sqlfileresume[] = "CREATE TABLE IF NOT EXISTS FILERESUME(Id INTEGER PRIMARY KEY AUTOINCREMENT , Path TEXT, Position INTEGER);";
...
void SQLiteDB::newResume(std::string path,int64_t position){
	char sqlquery[2048];
	char *err_msg = 0;
	sprintf(sqlquery,"INSERT INTO FILERESUME (Path,Position) VALUES(\"%s\",%lld);",path.c_str(),position);
	printf("INSERT: %s\n",sqlquery);
...
void SQLiteDB::updateResume(std::string path,int64_t position){
	char sqlquery[2048];
	char *err_msg = 0;
	sprintf(sqlquery,"UPDATE FILERESUME set Position = %lld  WHERE Path = \"%s\"",position,path.c_str());
	printf("UPDATE: %s\n",sqlquery);
	int rc = sqlite3_exec(db, sqlquery, 0, 0, &err_msg);
	if (rc != SQLITE_OK) {
		printf("DB Error: %s\n",err_msg);
...
#ifdef __SWITCH__
	rc = sqlite3_open_v2(zFilename,&pFile,SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE,"unix-dotfile");
#endif
#ifdef _WIN32
