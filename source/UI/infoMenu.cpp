    ...
    void InfoMenuWindow(bool *focus, bool *first_item) {
		...
		if (ImGui::Begin("Info Menu Window", nullptr, ImGuiWindowFlags_NoTitleBar|ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse)) {
			...
			if(libmpv != nullptr){
				...
				sprintf(ffmpegtext,"FFMPEG (MPV NOT INT)");
				sprintf(mpvtext,"MPV (MPV NOT INT)");
			}
			auto windowWidth = ImGui::GetWindowSize().x;
			ImGui::SetCursorPosX((windowWidth - imgloader->icons.NXMPBannerTexture.width) * 0.5f);
			GUI::NXMPImage((void*)(intptr_t)imgloader->icons.NXMPBannerTexture.id, ImVec2(imgloader->icons.NXMPBannerTexture.width,imgloader->icons.NXMPBannerTexture.height));
			...
			GUI::NXMPImage((void*)(intptr_t)imgloader->icons.FFMPEGTexture.id, ImVec2(imgloader->icons.FFMPEGTexture.width,imgloader->icons.FFMPEGTexture.height));
			...
			GUI::NXMPImage((void*)(intptr_t)imgloader->icons.MPVTexture.id, ImVec2(imgloader->icons.MPVTexture.width,imgloader->icons.MPVTexture.height));
